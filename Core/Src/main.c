...
bool button_debounce(uint32_t curmilli) {
	...
	return curmilli - encoder.last_pressed > debounce_time && curmilli - encoder.last_released > debounce_time;
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	...
	if (GPIO_Pin == GPIO_PIN_4 && button_debounce(curmilli)) {
		if (!encoder.pressed && HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_4) == GPIO_PIN_RESET) {
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (encoder.pressed && HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_4) == GPIO_PIN_SET && button_debounce(curmilli)) {
		  on_encoder_released(HAL_GetTick());
	  }

	  // Handle timers
	  if (!pomodoro.paused && (pomodoro.mode == FOCUSING || pomodoro.mode == RESTING)) {
